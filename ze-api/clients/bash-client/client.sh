...
# !
# ! Generated on: 2018-01-16T16:11:08.654Z
# !
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

#
# This is a Bash client for Ze API.
#
# LICENSE:
# 
#
...
operation_parameters_minimum_occurences["greetingWithNameGet:::greetingName"]=1
...
operation_parameters_maximum_occurences["greetingWithNameGet:::greetingName"]=0

##
# The type of collection for specifying multiple values for parameter:
# - multi, csv, ssv, tsv
declare -A operation_parameters_collection_type
operation_parameters_collection_type["greetingWithNameGet:::greetingName"]=""
...
$(tput bold)$(tput setaf 7)Ze API command line client (API version 1.0.0)$(tput sgr0)
...
EOF
    ...
    echo ""
    echo -e "$(tput bold)$(tput setaf 7)[greet]$(tput sgr0)"
read -d '' ops <<EOF
  $(tput setaf 6)greetingGet$(tput sgr0);the first service - greetings!
  $(tput setaf 6)greetingWithNameGet$(tput sgr0);Greeting with name
EOF
echo "  $ops" | column -t -s ';'
    echo ""
...
echo -e "              \t\t\t\t(e.g. 'https://localhost:8002')"
...
print_about() {
    echo ""
    echo -e "$(tput bold)$(tput setaf 7)Ze API command line client (API version 1.0.0)$(tput sgr0)"
    echo ""
    echo -e "License: "
    echo -e "Contact: martin.binder@qaware.de"
    echo ""
...
print_version() {
    echo ""
    echo -e "$(tput bold)Ze API command line client (API version 1.0.0)$(tput sgr0)"
    echo ""
}

##############################################################################
#
# Print help for greetingGet operation
#
##############################################################################
print_greetingGet_help() {
    echo ""
    echo -e "$(tput bold)$(tput setaf 7)greetingGet - the first service - greetings!$(tput sgr0)"
    echo -e ""
    echo -e "just say hello!" | fold -sw 80
    echo -e ""
    echo ""
    ...
    case 500 in
        1*)
        echo -e "$(tput setaf 7)  500;some error occured during service execution$(tput sgr0)" | column -t -s ';' | fold -sw 80 | sed '2,$s/^/       /'
        ;;
        2*)
        echo -e "$(tput setaf 2)  500;some error occured during service execution$(tput sgr0)" | column -t -s ';' | fold -sw 80 | sed '2,$s/^/       /'
        ;;
        3*)
        echo -e "$(tput setaf 3)  500;some error occured during service execution$(tput sgr0)" | column -t -s ';' | fold -sw 80 | sed '2,$s/^/       /'
        ;;
        4*)
        echo -e "$(tput setaf 1)  500;some error occured during service execution$(tput sgr0)" | column -t -s ';' | fold -sw 80 | sed '2,$s/^/       /'
        ;;
        5*)
        echo -e "$(tput setaf 5)  500;some error occured during service execution$(tput sgr0)" | column -t -s ';' | fold -sw 80 | sed '2,$s/^/       /'
        ;;
        *)
        echo -e "$(tput setaf 7)  500;some error occured during service execution$(tput sgr0)" | column -t -s ';' | fold -sw 80 | sed '2,$s/^/       /'
        ;;
...
##############################################################################
#
# Print help for greetingWithNameGet operation
#
##############################################################################
print_greetingWithNameGet_help() {
    echo ""
    echo -e "$(tput bold)$(tput setaf 7)greetingWithNameGet - Greeting with name$(tput sgr0)"
    echo -e ""
    echo -e "$(tput bold)$(tput setaf 7)Parameters$(tput sgr0)"
    echo -e "  * $(tput setaf 2)greetingName$(tput sgr0) $(tput setaf 4)[String]$(tput sgr0) $(tput setaf 1)(required)$(tput sgr0)$(tput sgr0) - $(tput setaf 3) Specify as: greetingName=value$(tput sgr0)" \
        | fold -sw 80 | sed '2,$s/^/    /'
    echo ""
    echo -e "$(tput bold)$(tput setaf 7)Responses$(tput sgr0)"
    case 200 in
...
call_greetingGet() {
    ...
    if [[ $force = false ]]; then
        validate_request_parameters "/api/greeting" path_parameter_names query_parameter_names
    fi

    local path=$(build_request_path "/api/greeting" path_parameter_names query_parameter_names)
    local method="GET"
...
##############################################################################
#
# Call greetingWithNameGet operation
#
##############################################################################
call_greetingWithNameGet() {
    local path_parameter_names=()
    local query_parameter_names=(greetingName)

    if [[ $force = false ]]; then
        validate_request_parameters "/api/greetingWithName" path_parameter_names query_parameter_names
    fi

    local path=$(build_request_path "/api/greetingWithName" path_parameter_names query_parameter_names)
    local method="GET"
    local headers_curl=$(header_arguments_to_curl)
    if [[ -n $header_accept ]]; then
        headers_curl="${headers_curl} -H 'Accept: ${header_accept}'"
...
case $key in
    ...
    ;;
    greetingGet)
    operation="greetingGet"
    ;;
    greetingWithNameGet)
    operation="greetingWithNameGet"
    ;;
    *==*)
